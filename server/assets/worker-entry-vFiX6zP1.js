...
async function getServerEntry() {
  if (!serverEntryPromise) {
    serverEntryPromise = import("./server-s-qwSZVY.js").then((n) => n.a1).then(
      (m) => m.default ?? m
    );
  }
...
function handleInstitutionalVideoRequest(env2) {
  const videoUrl = getEnvString(env2, "INSTITUTIONAL_VIDEO_URL");
  if (!videoUrl) {
    return jsonResponse(
...
function handlePublicConfigRequest(env2) {
  const institutionalVideoUrl = getEnvString(env2, "INSTITUTIONAL_VIDEO_URL") ?? null;
...
async function handleContactProxyRequest(request, env2) {
  const runtimeEnv = getRuntimeEnv(env2);
  const base = getEnvString(runtimeEnv, "MAIL_BACKEND_INTERNAL_URL");
  if (!base) {
    return jsonResponse(
      {
        message: "Formulario de contato indisponivel nesta configuracao. Defina MAIL_BACKEND_INTERNAL_URL no servico navship-app ou aponte o dominio para navship-gateway."
      },
      { status: 503 }
    );
  }
  ...
  if (request.method === "OPTIONS") {
    ...
    const r2 = await fetch(targetUrl, {
      method: "OPTIONS",
      headers: origin ? { origin } : void 0
    });
    const headers = new Headers();
    headers.set("cache-control", "no-store");
    const copy = ["access-control-allow-origin", "access-control-allow-methods", "access-control-allow-headers", "access-control-max-age"];
    for (const key of copy) {
      const v = r2.headers.get(key);
      if (v) {
        headers.set(key, v);
      }
    }
    ...
    return new Response(null, { status: r2.status === 204 ? 204 : r2.status, headers });
  }
  ...
  try {
    ...
    return jsonResponse(
      {
        message: "Nao foi possivel contactar o servico de e-mail interno. Verifique MAIL_BACKEND_INTERNAL_URL e se navship-backend esta a correr."
      },
      { status: 502 }
    );
  }
